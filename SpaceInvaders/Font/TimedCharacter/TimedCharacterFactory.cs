...
namespace SpaceInvaders
{
    class TimedCharacterFactory
    {
        ...
        static public void Install(string pMessage, 
            float deltaTimeToTrigger, float delayTime, 
            float xPos, float yPos, float _scale = 1.0f,
            float red = 1.0f, float green = 1.0f, float blue = 1.0f)
        {
            ...
            for (int i = 0; i < pMessage.Length; i++)
            {
                ...
                TimedCharacterCmd pCmd = new TimedCharacterCmd(pCmd_old, pCharacter,
                    xPos, yPos, _scale,
                    red, green, blue);
